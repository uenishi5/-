...
import jp.ac.hcs.mbraw.weather_alert.Weather_alertData.LowerAlertData;
import jp.ac.hcs.mbraw.weather_alert.Weather_alertData.UpperAlertData;
	...
	public Weather_alertEntity getWeather_alertData() {
		...
		if (alert[1].equals("発表なし")) { // 警報がない場合、発表無しの要素をを入れてentityを返す
			...
			if (alertfirst == false) {
				...
				// colspanを取得する
				Elements th = day.select("th"); // thの部分を取得
				List<String> thlist = new ArrayList<>();
				for (Element col : th) {
					thlist.add(col.attr("colspan")); // colspanを取得し、格納
				}
			...
			while (!alertfirst) { // 1段目の表の要素が存在しない場合、処理を飛ばす
				...
				for (int i = 0; i < 10; i++) {
					final UpperAlertData upperAlertData = data.getUpperAlertList().get(i);
			...
			while (weatherflg) {
				...
				for (int i = 0; i < 4; i++) {
					final LowerAlertData lowerAlertData = data.getLowerAlertList().get(i);
