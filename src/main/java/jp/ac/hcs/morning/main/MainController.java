...
public class MainController {
	...
	public String getMainData(Model model) {
		...
		WeatherEntity weatherEntity = mainService.getWeatherData();
		model.addAttribute("WeatherEntity", weatherEntity);
		/** 占いデータを格納 */
		HoroscopeEntity horoscopeEntity = mainService.getMainHoroscopeData();
		model.addAttribute("HoroscopeEntity", horoscopeEntity);
		/** 札幌の警報を取得*/
		Weather_alertEntity weather_alertEntity = mainService.getMainWeather_alertData();
		model.addAttribute("Weather_alertEntity", weather_alertEntity);
		/** ビットコインチャートを取得 */
		ChartEntity chartEntity = mainService.getMainChartData();
		model.addAttribute("ChartEntity", chartEntity);
		/** 札幌市のバス情報取得 */
		TrafficEntity trafficEntity = mainService.getMainTrafficData();
